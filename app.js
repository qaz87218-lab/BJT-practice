(() => {
  ...
  function renderTitles(){const root=document.getElementById('view-titles'),lp=levelProgress(),current=mainTitleAt(lp.level),eq=equippedSpecial(),unlocked=new Set(state.game.unlockedTitles||[]);const specials=GAME.specialTitles||[];root.innerHTML=`<section class="card title-hall-hero"><span class="player-kicker">TITLE HALL</span><div class="title-hall-current"><div><strong>Lv.${lp.level}</strong><h2>${esc(current.name)}</h2><p>${esc(current.desc)}</p></div><div class="equipped-title-box"><small>目前裝備異名</small><b>${eq?`《${esc(eq.name)}》`:'—'}</b>${eq?'<button class="text-btn" id="unequipTitle">卸下</button>':''}</div></div><div class="xp-row"><div class="progress xp-progress"><i style="width:${lp.pct}%"></i></div><b>${lp.level>=100?'MAX':`${lp.into} / ${lp.need} EXP`}</b></div></section><div class="section-title"><div><h2>主位階</h2><p>每 5 級左右突破一次；主位階不能手動更換。</p></div></div><div class="rank-timeline">${(GAME.mainTitles||[]).map(t=>{const have=t.level<=lp.level;return `<article class="rank-card ${have?'unlocked':'locked'} ${t.name===current.name?'current':''}"><span>Lv.${t.level}</span><b>${have?esc(t.name):'????'}</b><small>${have?esc(t.desc):'尚未取得'}</small></article>`}).join('')}</div><div class="section-title"><div><h2>異名收藏</h2><p>Boss、Combo、錯題復仇與特殊條件解鎖；已取得的異名可以自由裝備。</p></div><span class="small">${unlocked.size} / ??</span></div><div class="special-title-grid">${specials.map(t=>{const have=unlocked.has(t.id);return `<article class="card special-title-card rarity-${have?String(t.rarity||'').toLowerCase():'locked'} ${have?'unlocked':'locked'}"><div class="special-title-top"><span>${have?esc(rarityLabel(t.rarity)):'???'}</span><small>${have?(t.group==='boss'?'BOSS':'ACHIEVEMENT'):'???'}</small></div><h3>${have?`《${esc(t.name)}》`:'《????》'}</h3><p>${have?esc(t.desc):'取得後揭曉。'}</p>${have?`<button class="btn ${state.game.equippedTitle===t.id?'primary':''}" data-equip-title="${esc(t.id)}">${state.game.equippedTitle===t.id?'已裝備':'裝備異名'}</button>`:'<div class="locked-label">未取得</div>'}</article>`}).join('')}</div>`;root.querySelectorAll('[data-equip-title]').forEach(b=>b.onclick=()=>equipTitle(b.dataset.equipTitle));const un=document.getElementById('unequipTitle');if(un)un.onclick=()=>equipTitle(null)}
